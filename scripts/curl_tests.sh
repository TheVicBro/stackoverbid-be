...
# Helper: generate a future/past UTC timestamp
future_ts() {
  python3 -c "
...
# Helper: pull a value from JSON (avoids needing jq)
json_val() {
...
END_TIME=$(future_ts 5)

RESP=$(curl -s -X POST "$BASE_URL/auction/items" \
  -H "Content-Type: application/json" \
...
curl -s -X PATCH "$BASE_URL/auction/items/$EDIT_ITEM_ID" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SELLER_TOKEN" \
...
curl -s -X GET "$BASE_URL/catalogue/items" \
...
curl -s -X GET "$BASE_URL/catalogue/items?keyword=Guitar" \
  -H "Authorization: Bearer $BIDDER1_TOKEN"
echo ""
...
curl -s -X GET "$BASE_URL/catalogue/items/$ITEM_ID" \
  -H "Authorization: Bearer $BIDDER1_TOKEN"
echo ""
...
curl -s -X POST "$BASE_URL/auction/items/$ITEM_ID/bid" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $BIDDER1_TOKEN" \
  -d '{"amount":550}'
echo ""
...
curl -s -X POST "$BASE_URL/notifications/items/$ITEM_ID/broadcast-end" \
  -H "Authorization: Bearer $SELLER_TOKEN"
echo ""
...
RESP=$(curl -s -X POST "$BASE_URL/payment/items/$ITEM_ID/pay" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $BIDDER1_TOKEN" \
...
curl -s -X GET "$BASE_URL/payment/orders/$ORDER_ID/receipt" \
  -H "Authorization: Bearer $BIDDER1_TOKEN"
echo ""
...
curl -s -X GET "$BASE_URL/catalogue/items/1" \
...
curl -s -X POST "$BASE_URL/auction/items" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SELLER_TOKEN" \
  -d "{\"title\":\"   \",\"description\":\"Something\",\"starting_price\":10,\"end_time\":\"$(future_ts 3600)\"}"
echo ""
...
curl -s -X POST "$BASE_URL/auction/items" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SELLER_TOKEN" \
  -d "{\"title\":\"Bad\",\"description\":\"Negative\",\"starting_price\":-5,\"end_time\":\"$(future_ts 3600)\"}"
echo ""
...
curl -s -X POST "$BASE_URL/auction/items" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SELLER_TOKEN" \
  -d "{\"title\":\"Bad\",\"description\":\"Past\",\"starting_price\":10,\"end_time\":\"$(past_ts 3600)\"}"
echo ""
...
curl -s -X POST "$BASE_URL/auction/items/$EDIT_ITEM_ID/bid" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $BIDDER1_TOKEN" \
  -d '{"amount":5}'
echo ""
...
# ── Payment errors ───────────────────────────────────────────────────────────
echo ""
echo "--- Non-winner tries to pay (expect 403) ---"
curl -s -X POST "$BASE_URL/payment/items/$ITEM_ID/pay" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $BIDDER2_TOKEN" \
...
curl -s -X GET "$BASE_URL/payment/orders/99999/receipt" \
  -H "Authorization: Bearer $BIDDER1_TOKEN"
echo ""
